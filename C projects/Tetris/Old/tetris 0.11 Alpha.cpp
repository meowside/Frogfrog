...
char name[] = "Tetris 0.11 Beta - by David Knieradl";
const char *song[] = {
...
void GameType(){
	...
	while(true){
		...
		else if((keys.PRESSED == keys.DOWN || keys.PRESSED == 's') && selection.GAMETYPE < 5){
			selection.GAMETYPE++;
		}
...
void Game(){
	...
	if(selection.GAMETYPE == 1 || selection.GAMETYPE == 2 || selection.GAMETYPE == 5){
		while(!end){
...
void BoardWrite(){
	...
	if (selection.GAMETYPE == 1) FullRowDetection();
	else if(selection.GAMETYPE == 2) FullRowDetectionChain();
}
	...
	while(true){
		...
		while(page.GAMETYPE){
			...
			while(page.GAMETYPE){
				...
				Mid(selection.GAMETYPE == 5 ? "> Back <" : "  Back  ", (80 - strlen("> Back <")) / 2, screen.GAME.GAMETYPE.Y + 6, selection.GAMETYPE == 5 ? WHITE : DARKGRAY);
