...
void EpicText(const char TEXT[], int y);
void Game();
void Options();
void Info();
int Quit();
void SelectionMenu(int x, int y, int up_down, int sel);
void Mid(const char TEXT[], int y);
...
int brd_x = (80 - 18)/2, brd_y = 1;

bool board[23][18] = {false};
bool board2[23][18] = {false};

int menu_pos[] = {16,17,18,20};
int menu_str_len[] = {4,8,4,4};
	...
	while(true){
		...
		for(int i = 0; i < 32; i++){
			...
			if(_kbhit()){
				...
				if(key == 'w' && select > 0){
					select--;
					SelectionMenu(menu_str_len[select], menu_pos[select], 0, select);
				}
				else if(key == 's' && select < 3){
					select++;
					SelectionMenu(menu_str_len[select], menu_pos[select], 1, select);
				}
		...
		while(!end){
				...
				if(_kbhit()){
					...
					if(key == 'a' && pos_x > 1){
						...
						for(int i = 0; i < height; i++){
							...
							crmove(brd_x + pos_x, brd_y + pos_y);
							
							for(int j = 0; j < width + 1; j++){
								printf(board2[pos_y - i][pos_x + j] ? "#" : (board[pos_y - i][pos_x + j] ? "x" : ".") );
							}
						}
					...
					//
						...
						for(int i = 0; i < height; i++){
							...
							for(int j = 0; j < width + 1; j++){
								printf(board2[pos_y - i][pos_x + j - 1] ? "#" : (board[pos_y - i][pos_x + j] ? "x" : ".") );
							}
						}
					}
					...
					//
						...
						if (pos_x == 1) pos_x++;
						else if (pos_x == 17) pos_x--;
						
						
						for(int i = 0; i < height; i++){
							
							crmove(brd_x + pos_x, brd_y + pos_y - i);
			...
			//
				...
				for(int row = 0; row < height + 1; row++){
					...
					crmove(brd_x + pos_x, brd_y + pos_y - row);
					
					for(int column = 0; column < width; column++){
						board2[pos_y - row][pos_x + column] = board2[pos_y - row - 1][pos_x + column];
						board2[pos_y - row - 1][pos_x + column] = false;
						
						printf(board2[pos_y - row][pos_x + column] ? "#" : (board[pos_y - row][pos_x + column] ? "x" : ".") );
					}
				}
...
int Quit(){
	...
	while(true){
		...
		Mid("Are you sure you want to quit this game?", 12);
		printf("\n\n");
...
void SelectionMenu(int x, int y, int up_down, int sel){
	if(up_down == 0){
		crmove((80 - menu_str_len[sel + 1]) / 2 - 2, menu_pos[sel + 1]);
		printf(" ");
		
		crmove((80 + menu_str_len[sel + 1]) / 2 + 1, menu_pos[sel + 1]);
		printf(" ");
	}
	else{
		crmove((80 - menu_str_len[sel - 1]) / 2 - 2, menu_pos[sel - 1]);
	...
	crmove((80 - menu_str_len[sel]) / 2 - 2, menu_pos[sel]);
	printf(">");
		
	crmove((80 + menu_str_len[sel]) / 2 + 1, menu_pos[sel]);
	printf("<");
