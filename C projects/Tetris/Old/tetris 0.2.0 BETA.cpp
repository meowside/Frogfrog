...
char name[] = "Tetris 0.2.0 Beta - by David Knieradl";
...
int main(){
	...
	while(true){
		...
		else if((keys.PRESSED == keys.KEY[DOWN] || keys.PRESSED == 's') && selection.MAINMENU < 3){
			selection.MAINMENU++;
			keys.PRESSED = 0;
		}
			...
			if(selection.GAMETYPE != 7){
				...
				if(selection.GAMETYPE == 1){ page.GAMETYPE = false; Game(); }
					...
					while(page.MUTATOR){
						...
						else if((keys.PRESSED == keys.KEY[DOWN] || keys.PRESSED == 's') && selection.MUTATOR < 11){ selection.MUTATOR++; }
						...
						if(timer == 0){
							for(row = 0; row < 8; row++){
								for(column = 0; column < SMALL_W; column++){
									...
									help[SMALL_H - 1 - row][column] = (rand() % 2 == 0 ? false : true);
...
void GameOver(){
	...
	do{
		...
		else if(keys.PRESSED == 8 && namecount > 1){
			keys.PRESSED = 0;
			namecount--;
		} 
...
void Render(){
	...
	while(true){
		...
		while(page.SETTINGS){
			...
			if(xd == 40){
				Mid("                  ", (80 - strlen("DEFAULTS RESTORED!")) / 2, 10, LIGHTRED);
				defaults = false;
				xd = 0;
			} 
		...
		while(page.INFO){
			...
			while(page.HELP){
				...
				if(selection.HELPPAGE == MUTATORS){
					for(row_th = 0; row_th < SMALL_H; row_th++) Mid("            ", 10, 10 + row_th, BLACK);
					for(row_th = 0; row_th < 14; row_th++) Mid(img[row_th], 10, 10 + row_th, LIGHTRED);
				}
				
				else if(selection.HELPPAGE != SHARPSHOOTER){
					...
					for(row_th = 0; row_th < SMALL_H; row_th++){
						for(column_th = 0; column_th < SMALL_W - 2; column_th++){
							Mid(help2[row_th][column_th + 1] ? "#" : (help[row_th][column_th + 1] ? "x" : "."), 10 + column_th, 10 + SMALL_H - 1 - row_th, help2[row_th][column_th + 1] ? help2_color[row_th][column_th + 1] : help_color[row_th][column_th + 1]);
						}
					}
				}
