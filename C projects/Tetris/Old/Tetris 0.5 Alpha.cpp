...
#define UP_K 296				//keys.UP key code
#define DOWN_K 304				//keys.DOWN key code
#define LEFT_K 299				//keys.LEFT key code
#define RIGHT_K 301				//keys.RIGHT key code
...
void Render(int GAME, int PAUSE);
void Pause();
...
int temp = 0, last_width = 80, last_height = 26;
...
int main(){
	...
	while(true){
		...
		for(int row = 0; row < 48; row++){
				...
				crmove(0,0);
				textcolor(DARKGRAY);
				printf("sel %d | pressed %3d | keys.UP %3d | keys.DOWN %3d | pos_t %d | width %3d | height %2d |", select, key, keys.UP, keys.DOWN, position, last_width, last_height);
				Mid("DEBUG MODE",0,25,WHITE);
			#endif
			
			if(row % 4 == 0) WindowSize();
...
void Render(int COLOR, int PAUSE){
	...
	if(changed){
		...
		for(row = 0; row < 25; row++){
			if(row < 23) Mid("<|                |>", screen.GAME.BOARD.X - 2, screen.GAME.BOARD.Y + row, COLOR);
			else if(row == 23) Mid("<==================>", screen.GAME.BOARD.X - 2, screen.GAME.BOARD.Y + row, COLOR);
			else Mid(" vvvvvvvvvvvvvvvvvv ", screen.GAME.BOARD.X - 2, screen.GAME.BOARD.Y + row, COLOR);
		}
	...
	for(row = 0; row < 23; row++){
		for(column = 0; column < 16; column++){
			Mid(board2[row][column + 1] ? "#" : (board[row][column + 1] ? "x" : "."), screen.GAME.BOARD.X + column, screen.GAME.BOARD.Y + row, COLOR);
		}
	}
...
void OnLoad(){
	...
	screen.INFO.Y = 13;
	screen.GAME.BOARD.Y = 1;
	screen.GAME.BOARD.X = 10;
	screen.GAME.CURBLOCK.Y = 7;
	screen.GAME.CURBLOCK.X = 50;
	screen.GAME.PAUSE.Y = 15;
		...
		if((key == keys.UP || key == 'w') && select > 0){
			...
			if(select == 0){
				...
				while(true){
					...
					Mid(sel == 4 ? "> Rotate keys.LEFT" : "  Rotate keys.LEFT", (80 - strlen("Keys")) / 2 - 1, 18, sel == 4 ? WHITE : DARKGRAY);
					crmove(53, 18);
					printf("%c", Keys(keys.ROTATE_L));
					crmove(63, 18);
					printf("Q");
					
					Mid(sel == 5 ? "> Rotate keys.RIGHT" : "  Rotate keys.RIGHT", (80 - strlen("Keys")) / 2 - 1, 19, sel == 5 ? WHITE : DARKGRAY);
					crmove(53, 19);
...
int Quit(){
	...
	while(true){
		...
		Mid("Are you sure you want to quit this game?", (80 - strlen("Are you sure you want to quit this game?")) / 2, 10, WHITE);
		
		Mid(exit == 0 ? "> YES <" : "  YES  ", 20, 13, exit == 0 ? WHITE : DARKGRAY);
		Mid(exit == 1 ? "> No <" : "  No  ", 54, 13, exit == 1 ? WHITE : DARKGRAY);
		...
		if((key == keys.LEFT || key == 'a') && exit > 0){
			exit--;
		}
		
		if((key == keys.RIGHT || key == 'd') && exit < 1){
			exit++;
		}
