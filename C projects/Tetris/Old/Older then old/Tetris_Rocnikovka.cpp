...
int pos_x = 0, pos_y = 0, width = 0, heigth = 0, temp;
		...
		while(!end){
			...
			for(int i = 0; i < 16; i++){
				...
				if(_kbhit()){
					...
					//
						...
						bool mezipole[heigth][width] = {false};
						
						for(int a = 0; a < heigth; a++){
							for(int b = 0; b < width; b++){
								mezipole[a][b] = board2[pos_y - heigth + 1 + b][pos_x - 1 + width - a];
							}
						}
						...
						for(int i = 0; i < width; i++){
							for(int j = 0; j < heigth; j++){
								board2[pos_y - heigth + 1 + i][pos_x + j] = mezipole[i][j];
							}
						}
					...
					//
						...
						bool mezipole[width][heigth] = {false};
						
						for(int a = 0; a < width; a++){
							for(int b = 0; b < heigth; b++){
								mezipole[a][b] = board2[pos_y - b][pos_x + a];
								printf(mezipole[a][b] == true ? "x" : ".");
								Sleep(200);
							}
							printf("\n");
						}
						
						printf("\n h %d  w %d",heigth,width);
						temp = heigth;
						heigth = width;
						width = temp;
						printf("\n h %d  w %d\n\n",heigth,width);
				...
				/*if(render == true){
					...
					for(int row = 0; row < 22; row++){
						for(int column = 0; column < 16; column++){
							printf(board2[row + 1][column + 1] == true ? "#" : (board[row + 1][column + 1] == true ? "x" : "." ));
						}
						printf("\n");
					}
			...
			for(int row = 0; row < heigth; row++){
				for(int column = 0; column < width; column++){
					if(board2[pos_y - row][pos_x + column] == true && board[pos_y - row + 1][pos_x + column] == true){
						collided = true;
						brk = true;
						break;
					}
			...
			if(collided){
				...
				for(int row = 0; row < heigth; row++){
					for(int column = 0; column < width; column++){
						if(board2[pos_y - row][pos_x + column] == true){
