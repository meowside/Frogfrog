...
bool board3[LARGE_H + 1][LARGE_W] = {false};
...
void Game(){
	...
	memset(board3, false, sizeof(board3[0][0]) * LARGE_H * LARGE_W);
		...
		while(!collided){
			for(int i = 0; i < 30 && !collided; i++){
				...
				if(_kbhit()){
					...
					else if((keys.PRESSED == keys.KEY[RIGHT] || keys.PRESSED == 'd') && block.POS.X + block.WIDTH - 1 < screen.GAME.BRDSIZE.X - 2){
					...
					//
						...
						if(block.POS.X + block.WIDTH > screen.GAME.BRDSIZE.X - 1) canrotate = false;
...
void GenerateHelper(){
	...
	memcpy(board3, board2, sizeof(board2[0][0]) * LARGE_H * LARGE_W);
	helper = block;
	...
	while(!coll){
		...
		if(helper.POS.Y >= screen.GAME.BRDSIZE.Y - 1) coll = true;
		
		for(row = 0; row < helper.HEIGHT && !coll; row++){
			for(column = 0; column < helper.WIDTH && !coll; column++){
				if(board3[helper.POS.Y - row][helper.POS.X + column] && board[helper.POS.Y - row + 1][helper.POS.X + column]) coll = true;
			}
		}
		...
		if(!coll){
			...
			for(row = 0; row < helper.HEIGHT + 1; row++){
				for(column = 0; column < helper.WIDTH; column++){
					board3[helper.POS.Y - row][helper.POS.X + column] = board3[helper.POS.Y - row - 1][helper.POS.X + column];
					board3[helper.POS.Y - row - 1][helper.POS.X + column] = false;
				}
			}
		}
...
bool EndGame(){
	...
	for(column = screen.GAME.BRDSIZE.X / 2 - 3; column < screen.GAME.BRDSIZE.X / 2 + 3; column++){
		if(board[1][column]) return true;
	}
...
void Endless(){
	...
	for(row = screen.GAME.BRDSIZE.Y / 2; row > screen.GAME.BRDSIZE.Y / 2 - 4; row--){
		for(column = 0; column < screen.GAME.BRDSIZE.X; column++){
			if(board[row][column]) temp = true;
		}
	}
	...
	while(temp){
		...
		for(column = 0; column < screen.GAME.BRDSIZE.X; column++){
			board[screen.GAME.BRDSIZE.Y - 1][column] = false;
			board_color[screen.GAME.BRDSIZE.Y - 1][column] = DARKGRAY;
		}
		
		for(int i = screen.GAME.BRDSIZE.Y - 1; i > 1; i--){
...
void FullRowDetectionChain(){
	...
	do{
		...
		if(doit){
			for(row = screen.GAME.BRDSIZE.Y - 1; row >= 0; row--){
				for(column = 0; column < screen.GAME.BRDSIZE.X - 2; column++){
...
void FullRowDetectionHelp(){
	...
	for(row = 0; row <= blockhelp.POS.Y; row++){
		...
		if(get == SMALL_W - 2){
			for(column = 0; column < SMALL_W; column++) help[row][column] = false;
		...
		if(doit){
			for(row = SMALL_H - 1 - 1; row >= 0; row--){
				for(column = 0; column < SMALL_W - 2; column++){
					while(help[row][column + 1] && !help[row + 1][column + 1] && row < SMALL_H){
		...
		if((keys.PRESSED == keys.KEY[UP] || keys.PRESSED == 'w') && selection.INFO > 0){
			...
			if(selection.INFO == 0){
				...
				while(page.HELP){
					...
					if(timer == 0){
						memset(help, false, sizeof(board[0][0]) * SMALL_H * SMALL_W);
						memset(help2, false, sizeof(board2[0][0]) * SMALL_H * SMALL_W);
						
						memset(help_color, DARKGRAY, sizeof(board_color[0][0]) * SMALL_H * SMALL_W);
						memset(help2_color, DARKGRAY, sizeof(board2_color[0][0]) * SMALL_H * SMALL_W);
					}
						...
						if(timer == 0){
							for(row = 0; row < 8; row++){
								for(column = 0; column < SMALL_W; column++){
									...
									help[SMALL_H - 1 - row][column] = (Random(0, 1) == 0 ? false : true);
									
									if(help[SMALL_H - 1 - row][column]){
										int blockis = Random(0, 15);
										while(blockis != BLUE && blockis != GREEN && blockis != CYAN && blockis != RED && blockis != MAGENTA && blockis != LIGHTBLUE && blockis != LIGHTGREEN) blockis = Random(0, 15);
										
										help_color[SMALL_H - 1 - row][column] = blockis;
									}
								}
							}
			...
			if(page.GAMELOAD){
				...
				for(row_th = 0; row_th < screen.GAME.BRDSIZE.Y + 2; row_th++){
					...
					if(row_th < screen.GAME.BRDSIZE.Y){
						...
						Mid("|>", screen.GAME.BOARD.X + screen.GAME.BRDSIZE.X - 2, screen.GAME.BOARD.Y + row_th, WHITE);
					}
					else if(row_th == screen.GAME.BRDSIZE.Y){
						Mid("<", screen.GAME.BOARD.X - 2, screen.GAME.BOARD.Y + row_th, WHITE);
						for(int x = 0; x < screen.GAME.BRDSIZE.X; x++) Mid("=", screen.GAME.BOARD.X + x - 1, screen.GAME.BOARD.Y + row_th, WHITE);
						Mid(">", screen.GAME.BOARD.X + screen.GAME.BRDSIZE.X - 1, screen.GAME.BOARD.Y + row_th, WHITE);
					}
					else{
						for(int x = 0; x < screen.GAME.BRDSIZE.X; x++) Mid("v", screen.GAME.BOARD.X + x - 1, screen.GAME.BOARD.Y + row_th, WHITE);
					}
			...
			if(!mods.UPSIDEDOWN){
				for(row_th = 0; row_th < screen.GAME.BRDSIZE.Y; row_th++){
					for(column_th = 0; column_th < screen.GAME.BRDSIZE.X - 2; column_th++){
						Mid(board2[row_th][column_th + 1] || board3[row_th][column_th + 1] ? "#" : (board[row_th][column_th + 1] ? "x" : "."), screen.GAME.BOARD.X + column_th, screen.GAME.BOARD.Y + row_th, board2[row_th][column_th + 1] ? board2_color[row_th][column_th + 1] : (board3[row_th][column_th + 1] ? LIGHTGRAY : board_color[row_th][column_th + 1]));
					}
				}
			}
					...
					if(selection.HELPPAGE != SHARPSHOOTER){
						for(row_th = 0; row_th < SMALL_H; row_th++){
							for(column_th = 0; column_th < SMALL_W - 2; column_th++){
								Mid(help2[row_th][column_th + 1] ? "#" : (help[row_th][column_th + 1] ? "x" : "."), 10 + column_th, 9 + row_th, help2[row_th][column_th + 1] ? help2_color[row_th][column_th + 1] : help_color[row_th][column_th + 1]);
							}
						}
					}
						...
						for(row_th = 0; row_th < SMALL_H; row_th++){
							for(column_th = 0; column_th < SMALL_W - 2; column_th++){
								Mid(help2[row_th][column_th + 1] ? "#" : (help[row_th][column_th + 1] ? "x" : "."), 10 + column_th, 8 + SMALL_H - row_th, help2[row_th][column_th + 1] ? help2_color[row_th][column_th + 1] : help_color[row_th][column_th + 1]);
							}
						}
					}
				}
		...
		while(page.PAUSE){
			...
			if(page.GAMELOAD){
				...
				if(mods.UPSIDEDOWN){
					for(row_th = 0; row_th < screen.GAME.BRDSIZE.Y; row_th++){
						for(column_th = 0; column_th < screen.GAME.BRDSIZE.X - 2; column_th++){
							Mid(board2[screen.GAME.BRDSIZE.Y - 1 - row_th][column_th + 1] ? "#" : (board[screen.GAME.BRDSIZE.Y - 1 - row_th][column_th + 1] ? "x" : "."), screen.GAME.BOARD.X + column_th, screen.GAME.BOARD.Y + row_th, DARKGRAY);
						}
					}
				}
...
DWORD WINAPI TimerGB(void* data){
	while(page.GAME){
		...
		int X = Random(0, screen.GAME.BRDSIZE.X);
		int Y = Random(0, screen.GAME.BRDSIZE.Y - 1);
		
		board[Y][X] = false;
		board_color[Y][X] = DARKGRAY;
	}
}
