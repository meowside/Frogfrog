...
bool sound = false, changed = false, collided = false, qt_select = false;
...
char name[] = "Tetris 0.10 Beta - by David Knieradl";
const char *song[] = {
...
void Game(){
	...
	memset(board_color, 0, sizeof(board_color[0][0]) * 23 * 18);
	memset(board2_color, 0, sizeof(board2_color[0][0]) * 23 * 18);
				...
				if(_kbhit()){
					...
					if((keys.PRESSED == keys.LEFT || keys.PRESSED == 'a') && block.POS.X > 1){
						...
						for(row = block.POS.Y; row > block.POS.Y - block.HEIGHT; row--){
							if(board2[row][block.POS.X] && board[row][block.POS.X - 1]){
								canmove = false;
								break;
							} 
						}
					...
					//
						...
						bool left_collision = true, right_collision = true, coll = false;
						
						while(left_collision && right_collision){
							for(row = block.POS.Y; row > block.POS.Y - block.HEIGHT; row--){
								//if(board[row][block.POS.X + block.WIDTH - 1] && board2[row][block.POS.X + block.WIDTH - 1] || board[row][block.POS.X] && board2[row][block.POS.X - 1]) RotateLeft();
								
								if(board[row][block.POS.X + block.WIDTH - 1] && board2[row][block.POS.X + block.WIDTH - 1] || board[row][block.POS.X + block.WIDTH] && board2[row][block.POS.X + block.WIDTH]){
									right_collision = true;
								}
								
								else right_collision = false;
								
								if(board[row][block.POS.X] && board2[row][block.POS.X] ||board[row][block.POS.X - 1] && board2[row][block.POS.X - 1]){
									left_collision = true;
								}
							...
							else if(left_collision && right_collision){
								RotateLeft();
							}
						}
					...
					//
						...
						for(row = 0; row < block.HEIGHT; row++){
							for(column = 0; column < block.WIDTH; column++){
								if(board[block.POS.Y + column][block.POS.X + row] && board2[block.POS.Y + row][block.POS.X + row]){
									x++;
								}
							}
						}
						...
						for(row = 0; row < block.HEIGHT; row++){
							for(column = 0; column < block.WIDTH; column++){
								if(board[block.POS.Y + column][block.POS.X + row - x] && board2[block.POS.Y + row][block.POS.X + row]){
									canmoveleft = false;
								}
							}
						}
						...
						for(row = 0; row < block.HEIGHT; row++){
							for(column = 0; column < block.WIDTH; column++){
								if(board[block.POS.Y + column][block.POS.X + row + block.HEIGHT - 1 + x] && board2[block.POS.Y + row][block.POS.X + row]){
									canmoveright = false;
								}
							}
						}
						
						if(canmoveright && canmoveleft) RotateRight();
						else if(canmoveleft){
							RotateRight();
...
void MoveLeft(){
	for(row = 0; row < block.WIDTH; row++){
		for(column = 0; column < block.HEIGHT; column++){
			...
			board2_color[block.POS.Y - column][block.POS.X + row] = 0;
		}
	}
...
void MoveRight(){
	for(row = 0; row < block.WIDTH; row++){
		for(column = 0; column < block.HEIGHT; column++){
			...
			board2_color[block.POS.Y - column][block.POS.X - row + block.WIDTH - 1] = 0;
		}
	}
...
void RotateLeft(){
	...
	for(row = 0; row < block.HEIGHT; row++){
		for(column = 0; column < block.WIDTH; column++){
			...
			board2_color[block.POS.Y - block.HEIGHT + 1 + row][block.POS.X + column] = 0;
		}
	}
...
void MoveDown(){
	...
	for(row = 0; row < block.HEIGHT + 1; row++){
		for(column = 0; column < block.WIDTH; column++){
			...
			board2_color[block.POS.Y - row - 1][block.POS.X + column] = 0;
		}
	}
}
...
void BoardWrite(){
	for(row = 0; row < block.HEIGHT; row++){
		for(column = 0; column < block.WIDTH; column++){
			...
			board2_color[block.POS.Y - row][block.POS.X + column] = 0;
		}
	}
...
bool Pause(){
	...
	while(true){
		...
		if((keys.PRESSED == 'w' || keys.PRESSED == keys.UP) && selection.PAUSE > 0){
			...
			//else if(selection == 1) 
			else if(selection.PAUSE == 2){
				page.PAUSE = false;
				Options();
			}
			else if(selection.PAUSE == 3){
				page.PAUSE = false;
				Info();	
			} 
...
void Music(){
	...
	while(sound){
		...
		for(int i = 0; i < song_l[cur_song] * 10 && sound && keys.PRESSED != 'n' && keys.PRESSED != keys.NEXTSONG; i++){
			Sleep(100);
		}
