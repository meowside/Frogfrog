...
short changelog_rows[] = { 4, 2, 11, 8, 2};
...
int song_l[]	=	{ 121, 167, 210, 153, 208, 170, 207, 128, 81, 220 };
...
short help_txt_rows[]	=	{ 7, 3, 5, 5, 5, 5, 5, 5 };
				...
				if(selection.GAMETYPE == 1){ page.GAMETYPE	=	false; Game(); }
					...
					while(page.MUTATOR){
						...
						if((keys.PRESSED == keys.KEY[UP] || keys.PRESSED == 'w') && selection.MUTATOR > 0){ selection.MUTATOR--; }
							...
							else if(selection.MUTATOR == 5){ mods.TIMERUN -= 1; if(mods.TIMERUN < -1) mods.TIMERUN	=	60; }						
...
void Game(){
	...
	while(!end){
		...
		block.NEXT	=	Random(1, BLOCKS + 3);
		while(block.NEXT > BLOCKS && !customblocks[block.NEXT - BLOCKS - 1].IsActive) block.NEXT	=	Random(1, BLOCKS + 3);
	...
	if(sel == 1){
		...
		for(row = 0; row < 2; row++){
			for(column = 0; column < 4; column++){
				if(customblocks[0].Layout[row][column].IsBlock){
					brd -> Layers[blck -> POS.Y + row - 1][blck -> POS.X + column][1].IsBlock = customblocks[0].Layout[row][column].IsBlock;
					brd -> Layers[blck -> POS.Y + row - 1][blck -> POS.X + column][1].Color = customblocks[0].Layout[row][column].Color;
				}
			}
		}
	}
		...
		for(row = 0; row < 2; row++){
			for(column = 0; column < 4; column++){
				if(customblocks[1].Layout[row][column].IsBlock){
					brd -> Layers[blck -> POS.Y + row - 1][blck -> POS.X + column][1].IsBlock = customblocks[1].Layout[row][column].IsBlock;
					brd -> Layers[blck -> POS.Y + row - 1][blck -> POS.X + column][1].Color = customblocks[1].Layout[row][column].Color;
				}
		...
		for(row = 0; row < 2; row++){
			for(column = 0; column < 4; column++){
				if(customblocks[2].Layout[row][column].IsBlock){
					brd -> Layers[blck -> POS.Y + row - 1][blck -> POS.X + column][1].IsBlock = customblocks[2].Layout[row][column].IsBlock;
					brd -> Layers[blck -> POS.Y + row - 1][blck -> POS.X + column][1].Color = customblocks[2].Layout[row][column].Color;
				}
...
void BuildGame(BOARD *brd){
	...
	block.NEXT	=	Random(1, BLOCKS);
	while(block.NEXT > BLOCKS && !customblocks[block.NEXT - BLOCKS].IsActive) block.NEXT	=	Random(1, BLOCKS + 3);
...
void GetBlockSizes(){
	...
	for(row = 0; row < 3; row++){
		...
		for(int i = 0; i < 2 && !CanGetSize; i++){
			for(int j = 0; j < 4 && !CanGetSize; j++){
				if(customblocks[row].Layout[i][j].IsBlock) CanGetSize = true;
			}
		}
		
		if(!CanGetSize) continue;
		
		for(int i = 0; i < 1; i++){
			if(!customblocks[row].Layout[i][0].IsBlock && !customblocks[row].Layout[i][1].IsBlock && !customblocks[row].Layout[i][2].IsBlock && !customblocks[row].Layout[i][3].IsBlock){
				customblocks[row].Layout[i][0].IsBlock = customblocks[row].Layout[i + 1][0].IsBlock;
				customblocks[row].Layout[i][1].IsBlock = customblocks[row].Layout[i + 1][1].IsBlock;
				customblocks[row].Layout[i][2].IsBlock = customblocks[row].Layout[i + 1][2].IsBlock;
				customblocks[row].Layout[i][3].IsBlock = customblocks[row].Layout[i + 1][3].IsBlock;
				
				customblocks[row].Layout[i][0].Color = customblocks[row].Layout[i + 1][0].Color;
				customblocks[row].Layout[i][1].Color = customblocks[row].Layout[i + 1][1].Color;
				customblocks[row].Layout[i][2].Color = customblocks[row].Layout[i + 1][2].Color;
				customblocks[row].Layout[i][3].Color = customblocks[row].Layout[i + 1][3].Color;
				
				customblocks[row].Layout[i + 1][0].IsBlock = false;
				customblocks[row].Layout[i + 1][1].IsBlock = false;
				customblocks[row].Layout[i + 1][2].IsBlock = false;
				customblocks[row].Layout[i + 1][3].IsBlock = false;
				
				customblocks[row].Layout[i + 1][0].Color = BLACK;
				customblocks[row].Layout[i + 1][1].Color = BLACK;
				customblocks[row].Layout[i + 1][2].Color = BLACK;
				customblocks[row].Layout[i + 1][3].Color = BLACK;
			}
			else break;
		}
		
		for(int j = 0; j < 3; j++){
			if(!customblocks[row].Layout[0][0].IsBlock && !customblocks[row].Layout[1][0].IsBlock){
				for(int i = 0; i < 3; i++){
					customblocks[row].Layout[0][i].IsBlock = customblocks[row].Layout[0][i + 1].IsBlock;
					customblocks[row].Layout[1][i].IsBlock = customblocks[row].Layout[1][i + 1].IsBlock;
					
					customblocks[row].Layout[0][i].Color = customblocks[row].Layout[0][i + 1].Color;
					customblocks[row].Layout[1][i].Color = customblocks[row].Layout[1][i + 1].Color;
					
					customblocks[row].Layout[0][i + 1].IsBlock = false;
					customblocks[row].Layout[1][i + 1].IsBlock = false;
					
					customblocks[row].Layout[0][i + 1].Color = BLACK;
					customblocks[row].Layout[1][i + 1].Color = BLACK;
				}
...
void BlockCreation(){
	...
	while(true){
		...
		if(keys.PRESSED == keys.KEY[UP] || keys.PRESSED == 'w'){
		...
		else if(keys.PRESSED == keys.KEY[DOWN] || keys.PRESSED == 's'){
			keys.PRESSED	=	0;
			if(selection.BLOCKCREATEX > 4 && selection.BLOCKSEL < 3)selection.BLOCKSEL++;
			...
			if(selection.BLOCKCREATEX < 4 && selection.BLOCKSEL < 3){
				...
				if(IsAvailable) customblocks[selection.BLOCKSEL].IsActive = !customblocks[selection.BLOCKSEL].IsActive;
...
void ChangeLog(){
	...
	while(true){
		...
		else if((keys.PRESSED == keys.KEY[DOWN] || keys.PRESSED == 's') && selection.CHANGELOG < 1){
			selection.CHANGELOG++;
			page.PAGELOAD = true;
		}
		
		else if((keys.PRESSED == keys.KEY[LEFT] || keys.PRESSED == 'a') && selection.CHANGELOG == 0){
			if(selection.CHANGELOGPAGE == 0) selection.CHANGELOGPAGE = 4;
			else selection.CHANGELOGPAGE--;
			
			page.PAGELOAD = true;
		}
		
		else if((keys.PRESSED == keys.KEY[RIGHT] || keys.PRESSED == 'd') && selection.CHANGELOG == 0){
			if(selection.CHANGELOGPAGE == 4) selection.CHANGELOGPAGE = 0;
			else selection.CHANGELOGPAGE++;
			
			page.PAGELOAD = true;
		}
		
		else if(keys.PRESSED == 13 && selection.CHANGELOG == 1){
...
bool KeyBindingChecker(){
	...
	if(keys.PRESSED == 13) return true;
...
void LoadBlocks(){
	...
	if(fp == NULL){
		...
		for(row	=	0; row < 3; row++) customblocks[row].Left	=	5;
...
void Render(){
	...
	while(true){
		...
		while(page.HIGHSCORE){
			...
			for(row_th	=	0; row_th < 10; row_th++){
				crmove(25, 10 + row_th);
				puts(highscore.players[row_th]);
				
				crmove(55 - Digits(highscore.scores[row_th]), 10 + row_th);
				printf("%d", highscore.scores[row_th]);
			}
		...
		while(page.GAME){
			...
			if(page.GAMELOAD){
				switch(block.NEXT){
					...
					case 8:
						for(row_th = 0; row_th < 2; row_th++){
							for(column_th = 0; column_th < 4; column_th++){
								Mid(customblocks[0].Layout[row_th][column_th].IsBlock ? "#" : " ", screen.GAME.CURBLOCK.X - 2 + column_th, screen.GAME.CURBLOCK.Y + 7 + row_th, DARKGRAY);
							}
						}
						break;
					...
					case 9:
						for(row_th = 0; row_th < 2; row_th++){
							for(column_th = 0; column_th < 4; column_th++){
								Mid(customblocks[1].Layout[row_th][column_th].IsBlock ? "#" : " ", screen.GAME.CURBLOCK.X - 2 + column_th, screen.GAME.CURBLOCK.Y + 7 + row_th, DARKGRAY);
							}
						}
						break;
					...
					case 10:
						for(row_th = 0; row_th < 2; row_th++){
							for(column_th = 0; column_th < 4; column_th++){
								Mid(customblocks[2].Layout[row_th][column_th].IsBlock ? "#" : " ", screen.GAME.CURBLOCK.X - 2 + column_th, screen.GAME.CURBLOCK.Y + 7 + row_th, DARKGRAY);
							}
						}
						break;
				...
				Mid("Custom #1", screen.GAME.CURBLOCK.X - strlen("Next block") + 2, screen.GAME.CURBLOCK.Y + 19, WHITE);
				Mid("Custom #2", screen.GAME.CURBLOCK.X - strlen("Next block") + 2, screen.GAME.CURBLOCK.Y + 20, WHITE);
				Mid("Custom #3", screen.GAME.CURBLOCK.X - strlen("Next block") + 2, screen.GAME.CURBLOCK.Y + 21, WHITE);
				
				for(row_th = 0; row_th < 10; row_th++){
					Mid("0000", screen.GAME.CURBLOCK.X + 4, screen.GAME.CURBLOCK.Y + 12 + row_th, DARKGRAY);
				}
				...
				crmove(screen.GAME.CURBLOCK.X + 8 - Digits(block.STATS.C1), screen.GAME.CURBLOCK.Y + 19);
				textcolor(WHITE, BLACK);
				printf("%d", block.STATS.C1);
				
				crmove(screen.GAME.CURBLOCK.X + 8 - Digits(block.STATS.C2), screen.GAME.CURBLOCK.Y + 20);
				textcolor(WHITE, BLACK);
				printf("%d", block.STATS.C2);
				
				crmove(screen.GAME.CURBLOCK.X + 8 - Digits(block.STATS.C3), screen.GAME.CURBLOCK.Y + 21);
				textcolor(WHITE, BLACK);
				printf("%d", block.STATS.C3);
			...
			if(mods.TIMERUN > 0){
				...
				printf("Time Left: %02d:%02d", timeleft / 600, timeleft % 600 / 10);
			}
		...
		while(page.SETTINGS){
			...
			while(page.KEYS){
				...
				sprintf(tmp, "%s", Keys(keys.KEY[UP]));
				Mid(tmp, 53, 14, selection.KEYS == 0 ? WHITE : DARKGRAY);
				Mid("W", 63, 14, selection.KEYS == 0 ? WHITE : DARKGRAY);
				
				Mid(selection.KEYS == 1 ? "> Down" : "  Down", (80 - strlen("Keys")) / 2 - 1, 15, selection.KEYS == 1 ? WHITE : DARKGRAY);
				sprintf(tmp, "%s", Keys(keys.KEY[DOWN]));
				Mid(tmp, 53, 15, selection.KEYS == 1 ? WHITE : DARKGRAY);
				Mid("S", 63, 15, selection.KEYS == 1 ? WHITE : DARKGRAY);
				
				Mid(selection.KEYS == 2 ? "> Left" : "  Left", (80 - strlen("Keys")) / 2 - 1, 16, selection.KEYS == 2 ? WHITE : DARKGRAY);
				sprintf(tmp, "%s", Keys(keys.KEY[LEFT]));
				Mid(tmp, 53, 16, selection.KEYS == 2 ? WHITE : DARKGRAY);
				Mid("A", 63, 16, selection.KEYS == 2 ? WHITE : DARKGRAY);
				
				Mid(selection.KEYS == 3 ? "> Right" : "  Right", (80 - strlen("Keys")) / 2 - 1, 17, selection.KEYS == 3 ? WHITE : DARKGRAY);
				sprintf(tmp, "%s", Keys(keys.KEY[RIGHT]));
				Mid(tmp, 53, 17, selection.KEYS == 3 ? WHITE : DARKGRAY);
				Mid("D", 63, 17, selection.KEYS == 3 ? WHITE : DARKGRAY);
				
				Mid(selection.KEYS == 4 ? "> Rotate left" : "  Rotate left", (80 - strlen("Keys")) / 2 - 1, 18, selection.KEYS == 4 ? WHITE : DARKGRAY);
				sprintf(tmp, "%s", Keys(keys.KEY[ROTATE_L]));
				Mid(tmp, 53, 18, selection.KEYS == 4 ? WHITE : DARKGRAY);
				Mid("Q", 63, 18, selection.KEYS == 4 ? WHITE : DARKGRAY);
				
				Mid(selection.KEYS == 5 ? "> Rotate right" : "  Rotate right", (80 - strlen("Keys")) / 2 - 1, 19, selection.KEYS == 5 ? WHITE : DARKGRAY);
				sprintf(tmp, "%s", Keys(keys.KEY[ROTATE_R]));
				Mid(tmp, 53, 19, selection.KEYS == 5 ? WHITE : DARKGRAY);
				Mid("E", 63, 19, selection.KEYS == 5 ? WHITE : DARKGRAY);
				
				Mid(selection.KEYS == 6 ? "> Drop" : "  Drop", (80 - strlen("Keys")) / 2 - 1, 20, selection.KEYS == 6 ? WHITE : DARKGRAY);
				sprintf(tmp, "%s", Keys(keys.KEY[DROP]));
				Mid(tmp, 53, 20, selection.KEYS == 6 ? WHITE : DARKGRAY);
				Mid("SPACE", 63, 20, selection.KEYS == 6 ? WHITE : DARKGRAY);
				
				Mid(selection.KEYS == 7 ? "> Pause" : "  Pause", (80 - strlen("Keys")) / 2 - 1, 21, selection.KEYS == 7 ? WHITE : DARKGRAY);
				sprintf(tmp, "%s", Keys(keys.KEY[PAUSE]));
				Mid(tmp, 53, 21, selection.KEYS == 7 ? WHITE : DARKGRAY);
				Mid("ESC", 63, 21, selection.KEYS == 7 ? WHITE : DARKGRAY);
				
				Mid(selection.KEYS == 8 ? "> Next song" : "  Next song", (80 - strlen("Keys")) / 2 - 1, 22, selection.KEYS == 8 ? WHITE : DARKGRAY);
				sprintf(tmp, "%s", Keys(keys.KEY[NEXTSONG]));
				Mid(tmp, 53, 22, selection.KEYS == 8 ? WHITE : DARKGRAY);
				Mid("N", 63, 22, selection.KEYS == 8 ? WHITE : DARKGRAY);
				...
				if(keychanging){ Mid("Press any key, DELETE for cancel", (80 - strlen("Press any key, DELETE for cancel")) / 2, 9, WHITE); }
				else if(keys.SAMEKEYEXCEPTION){ Mid(" Can't bind, key already binded!", (80 - strlen(" Can't bind, key already binded!")) / 2, 9, RED); }
		...
		while(page.INFO){
			...
			while(page.CHANGELOG){
				if(page.PAGELOAD){
					...
					EpicText("change", 2, LIGHTGREEN);
					
					crmove(40 - (strlen(changelogver[selection.CHANGELOGPAGE]) + 4) / 2, 10);
					textcolor(selection.CHANGELOG == 0 ? WHITE : DARKGRAY, BLACK);
					printf(selection.CHANGELOG == 0 ? "> %s <" : "  %s  ", changelogver[selection.CHANGELOGPAGE]);
					
					for(row_th = 0; row_th < changelog_rows[selection.CHANGELOGPAGE]; row_th++){
						Mid(changelog[selection.CHANGELOGPAGE][row_th], 27, 14 + row_th, DARKGRAY);
					}
					
					Mid(selection.CHANGELOG == 1 ? "> Back <" : "  Back  ", 40 - strlen("> Back <") / 2, 27, selection.CHANGELOG == 0 ? DARKGRAY : WHITE);
